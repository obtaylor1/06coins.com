...
export default function Reports() {
  ...
  return <AdminShell title="Reports" actions={<><button onClick={() => window.print()} className="flex h-9 items-center gap-2 border border-[#8B6B2B]/40 px-3 text-xs"><Printer className="h-4 w-4"/>Print / PDF</button><ExportButton filename={`06coins-${type}-${period}-days.csv`} rows={exportRows}/></>}>
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border border-[#8B6B2B]/30 bg-[#11110f] px-4 py-3">
      <p className="text-xs text-[#E8DEC2]/65">All totals come from recorded orders and operations data.</p>
      <label className="flex items-center gap-3 text-xs uppercase tracking-wider text-[#C8A856]">Date range<select value={period} onChange={event => setPeriod(event.target.value)} className="h-9 border border-[#8B6B2B]/40 bg-[#090909] px-3 text-xs normal-case tracking-normal text-[#E8DEC2]"><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option><option value="all">All time</option></select></label>
    </div>
    <div className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 border border-[#8B6B2B]/35 bg-[#11110f] p-4 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs uppercase tracking-[.14em] text-[#C8A856]/70">{period === "all" ? "All recorded activity" : `Last ${period} days`}</p><h2 className="mt-1 font-serif text-2xl">{title} report</h2></div>{data?.generatedAt && <p className="text-xs opacity-55">Generated {new Date(data.generatedAt).toLocaleString()}</p>}</div>
        {isLoading && <p className="grid h-64 place-items-center text-sm opacity-65">Preparing report…</p>}
        ...
        {data && <>
          <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{data.metrics.map((metric: any) => <div key={metric.label} className="border border-[#8B6B2B]/25 bg-[#0c0c0a] p-4"><small className="text-xs uppercase tracking-wider opacity-65">{metric.label}</small><p className="mt-2 break-words font-serif text-2xl text-[#E8DEC2]">{formatValue(metric.value, metric.format)}</p></div>)}</div>
          ...
          <div className="mt-6 overflow-x-auto border border-[#8B6B2B]/25"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-[#C8A856]/5 text-xs uppercase tracking-wider text-[#E8DEC2]/65"><tr>{data.columns.map((column:any) => <th key={column.key} className="p-3">{column.label}</th>)}</tr></thead><tbody>{data.rows.map((row:any, index:number) => <tr key={row.order || row.sku || row.email || `${type}-${index}`} className="border-t border-[#8B6B2B]/20">{data.columns.map((column:any) => <td key={column.key} className="max-w-xs truncate p-3">{formatValue(row[column.key], column.format)}</td>)}</tr>)}</tbody></table>{!data.rows.length && <div className="p-10 text-center"><p className="font-serif text-lg text-[#E8DEC2]/65">No recorded data for this report</p><p className="mt-2 text-xs text-[#E8DEC2]/55">The report will populate automatically when qualifying activity is recorded.</p></div>}</div>
        </>}
      </section>
    </div>
  </AdminShell>;
